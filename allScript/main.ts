...
function update_info(id) {
    ...
    let stranger_message = $('.flex_row');
    let stranger_message_id = stranger_message.prop('id');
    ...
    let last_message = $('.main_box_area_chat_message_item');
    let last_message_id = last_message.prop('id');

    let sub = last_message_id - stranger_message_id;
    ...
    let message_count: number = 20;
    if ((sub / 20) >= 1) {

        let count: number = Math.floor(sub / 20);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $(response).each(function (index, message_info) {
                ...
                let dt = new Date(message_info['created_at'] * 1000);
                let hours: any = dt.getHours();
                let minutes: any = dt.getMinutes();
...
function changeUserInfo() {
    ...
    let user_name: string = $('#user_name').val();
...
function sendFiles(files) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            let message_avatar = $('#message_avatar_' + user_id + '*');
...
function sendMessage() {
    ...
    let new_message_text: string = $('#new_message').val();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            let dt = new Date(response['created_at']);
            ...
            let edit = '<div class="main_box_area_chat_message_edit_area" id="edit_pannel_' + response['message_id'] + '">' +
            ...
            let message_item = $('<div class="main_box_area_chat_message_row flex_row_reverse" id="message_row_' + response['message_id'] + '">' +
                '<div class="main_box_area_chat_message_item flex_row_reverse"' +
...
function loadMessage() {
    ...
    let last_message = $('.main_box_area_chat_message_item').last();
    let last_message_id = last_message.prop('id');
...
function sendEditedMessage(message_id, expired_at) {
    ...
    let message_button = $('<button class="send_button" id="new_message_button"' +
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            let old_time = $('div#time_' + message_id);
...
function editMessage(message_id, expired_at) {
    ...
    let message = $('#message_' + message_id).html();

    $('#new_message').val(message);

    let new_message_button = $('<button class="send_button" id="new_message_button"' +
...
function deleteMessage(message_id, expired_at) {
    ...
    $.ajax({
        ...
        success: function () {
            ...
            let restor_message = $('<div class="restor_message" id="restore_' + message_id + '">' +
...
function restoreMessage(message_id) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            let dt = new Date(response['updated_at'] * 1000);
            let hours: any = dt.getHours();
            let minutes: any = dt.getMinutes();
            ...
            let restored_message = $('<div class="main_box_area_chat_message_row flex_row_reverse" id="message_row_' + response['message_id'] + '">' +
                '<div class="main_box_area_chat_message_item flex_row_reverse"' +
                'id="' + response['message_id'] + '">' +
                '<div class="main_box_item_avatar avatar50px" id="message_avatar_' + user_id + '"' +
                'style=\"' + avatar + '\">' +
